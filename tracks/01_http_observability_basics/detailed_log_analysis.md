...
**Root Cause:** A SQL statement generated by Hibernate attempted to select more than 1664 columns, exceeding PostgreSQL’s hard limit.
...
- **What it is:** Part of Spring’s AOP infrastructure  
...
- **Purpose:** Converts provider-specific exceptions (Hibernate, JPA) into Spring’s data-access hierarchy  
...
- **Purpose:** JVM’s native implementation used by reflection
...
↓
Returns error: “target lists can have at most 1664 entries”
↓
...
↓
GenericJDBCException → JpaSystemException
```
...
```java
// BillServiceImpl.java (lines 1161–1172)
...
2. **Problematic Entity Structure**  
   - Possibly 50–100+ relationships  
   - Each relationship adds 10–20 columns  
   - 100 relationships × 16 columns = 1600+ columns, hitting the limit
...
- [ ] Review `BillServiceImpl.generateBillsProvisionedInBatch()` around lines 1161–1172  
...
The numerous proxies and interceptors (AOP, transactions, exception translation) are typical in modern Spring applications—they add valuable features, but they also lengthen stack traces.
